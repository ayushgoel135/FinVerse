...
def enable_2fa(user_id):
    conn = init_db()
    c = conn.cursor()
    c.execute("UPDATE users SET two_fa_enabled = 1 WHERE user_id = ?", (user_id,))
    conn.commit()
...
class ReportGenerator:
    ...
    def generate_monthly_report(user_id, month, year):
        ...
        # Use standard Arial font (built into FPDF)
        pdf.set_font("Arial", size=12)

        # Header
        pdf.set_fill_color(37, 99, 235)  # Blue
        pdf.set_text_color(255, 255, 255)
        pdf.cell(0, 10, f"OptiSpend PRO Monthly Report - {calendar.month_name[month]} {year}", 0, 1, 'C', 1)
        pdf.ln(10)
        ...
        if not financial_data.empty:
            ...
            pdf.cell(0, 10, f"Income: Rs. {fin_data['income']:,.0f}", 0, 1)  # Changed to Rs. instead of Rs.
            pdf.cell(0, 10, f"Expenses: Rs. {fin_data['expenses']:,.0f}", 0, 1)
            pdf.cell(0, 10, f"Savings: Rs. {fin_data['savings']:,.0f}", 0, 1)

            if not prev_financial.empty:
                prev_fin = prev_financial.iloc[0]
                pdf.cell(0, 10,
                         f"Savings Change: Rs. {fin_data['savings'] - prev_fin['savings']:+,.0f} from previous month",
                         0, 1)
        ...
        if not health_data.empty:
            ...
            if not prev_health.empty:
                prev_h = prev_health.iloc[0]
                pdf.cell(0, 10,
                         f"Change: {health['total_score'] - prev_h['total_score']:+.0f} points from previous month", 0,
                         1)
        ...
        if not optimizations.empty:
            total_savings = (optimizations['current_amount'] - optimizations['optimized_amount']).sum()
        ...
        if not optimizations.empty:
            top_increases = optimizations.nlargest(3, 'optimized_amount')
            for _, row in top_increases.iterrows():
                pdf.cell(0, 10, f"- Consider increasing spending on {row['category']}", 0, 1)

            top_decreases = optimizations.nsmallest(3, 'optimized_amount')
        ...
        try:
            ...
            if isinstance(pdf_str, str):
                # First try latin1, fallback to utf-8 if needed
                try:
...
def show_personal_finance(user_id):
    ...
    with st.sidebar:
        ...
        st.session_state.financial_data['income'] = st.number_input(
            "Annual Income (₹)",
            min_value=0,
            value=int(st.session_state.financial_data.get('income', 0)),
            step=1000,
            key="sidebar_income"
        )
...
def show_main_app(user_id):
    ...
    # Navigation
    menu = ["Personal Finance", "Business Tools", "Reports", "Settings"]
    ...
    if choice == "Personal Finance":
        ...
        st.header("Settings")
        st.write("Account settings and preferences")
